...
function init() {
	...
	const keyboardSwiper = new Swiper(keyboardWrapper, {
		...
		pagination: {
			...
			renderBullet: function (index, className) {
				...
				return `<span class="${className}"><div class="text-gradient">${index}</div></span>`
			},
		},
	...
	function updateContentVisibility() {
		const activeSlideIndex = keyboardSwiper.activeIndex // Индекс активного слайда
		...
		// Изменение условия для отображения playSection
		if (activeSlideIndex >= 1) {
			// Появляется на 2 слайде (индекс 1)
			gsap.to(playSection, {
				visibility: 'visible',
				opacity: 1,
				duration: 0.5,
	...
	class Podcast {
		constructor() {
			this.intro = { maxCount: 10, currentCount: 0 }
			this.start = { maxCount: 6, currentCount: 0 }
			this.keySlider = {
		...
		introAnimate(direction) {
			if (direction > 0 && this.intro.currentCount < this.intro.maxCount) {
				this.intro.currentCount++
			} else if (direction < 0 && this.intro.currentCount > 0) {
				this.intro.currentCount--
			}

			const progress = this.intro.currentCount / this.intro.maxCount
			const yOffset = -100 * progress
			gsap.to('.new-podcast__section--intro', {
				y: `${yOffset}%`,
				duration: 0.5,
				ease: 'power2.out',
			})

			// Появление секции `start`, когда интро пройдено наполовину
			if (progress >= 0.5) {
				gsap.to('.new-podcast__section--start', {
					opacity: (progress - 0.5) * 2, // от 0 до 1 по мере достижения полной прокрутки интро
					duration: 0.5,
					ease: 'power2.out',
				})
			} else {
			...
			console.log(
				`Intro Animation Step: ${this.intro.currentCount}, Progress: ${progress}`
			)
		}

		startAnimate(direction) {
			if (direction > 0 && this.start.currentCount < this.start.maxCount) {
				this.start.currentCount++
			} else if (direction < 0 && this.start.currentCount > 0) {
				this.start.currentCount--
			}
			const opacityValue = 1 - this.start.currentCount / this.start.maxCount
			gsap.to('.new-podcast__section--start', {
			...
			const xOffset =
				-trackWidth * 0.5 * (this.start.currentCount / this.start.maxCount)
			const yOffset = -(
				Math.tan(angleInRadians) *
				trackWidth *
				0.5 *
				(this.start.currentCount / this.start.maxCount)
			)
			...
			console.log(`Start Animation Step: ${this.start.currentCount}`)
		}
		...
		masterAnimate(direction) {
			if (direction > 0) {
				if (this.intro.currentCount < this.intro.maxCount) {
					this.introAnimate(direction)
				} else if (this.start.currentCount < this.start.maxCount) {
				...
				if (this.keySlider.currentCount > 0) {
					this.keySliderAnimate(direction)
				} else if (this.start.currentCount > 0) {
					this.startAnimate(direction)
				} else {
					this.introAnimate(direction)
				}
